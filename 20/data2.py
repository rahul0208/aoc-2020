...
with open("data.txt", 'r') as file:
    ...
    for k, v in data.items():
        lines = v.split('\n')
        flips[k] = [lines, turn(lines, 1), turn(lines, 2), turn(lines, 3), [x[::-1] for x in lines],
                    turn([x[::-1] for x in lines], 1), turn([x[::-1] for x in lines], 2),
                    turn([x[::-1] for x in lines], 3)]
        borders[k] = [x[0] for x in flips[k]]
    amt_borders = {k: len([z for z in v if any([z in w for q, w in borders.items() if k != q])]) for k, v in
                   borders.items()}
    print('Part 1: {}'.format(
        reduce(lambda x, y: x * y, [a for a, b in amt_borders.items() if b == min(amt_borders.values())])))
